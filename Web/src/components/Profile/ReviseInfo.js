...
import "../../styles/ReviseInfo.css"
import { userRevise } from '../Reducer/userSlice';
...
function ReviseInfo() {
    ...
    const server_revise_url = 'http://218.232.159.156:10081/member/register';
    ...
    const onClickReviseInfo = (e) => {
        ...
        axios.post(server_revise_url, JSON.stringify(userRevisedata), {
            headers: {
            ...
            if (res.data.result === 'success') {
                alert('수정 성공');
                // dispatch(userRevise(userRevisedata));
                //Home으로 이동
                navigate('/Profile');
            }
            //회원가입 실패
    ...
    return (
        <div>
            ...
            {/* 회원가입버튼 */}
            <span>
                <button type='button' onClick={onClickReviseInfo}>SignIn</button>
            </span>
